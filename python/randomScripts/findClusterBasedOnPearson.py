...
df = pd.read_csv('data/muscleTFStuff/muscleCluster_overall_correlationBetweenTFAndCluster.csv', index_col=0)
...
for index, value in enumerate(max):
    max[index] = value.replace('cluster', '')
...
with open('data/muscleTFStuff/muscleCluster_clusterBasedOnPearson.txt', 'w') as outfile:
